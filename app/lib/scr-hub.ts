      ...
      <div class="split rail-320">
        <div class="stack" style="gap:14px">
          ${feed.length ? feed.map((f, i) => `
            ...
            </div>`).join("") : `<div class="card"><div class="empty"><div class="ill">${IC.bell}</div><h4>${t("hub.feedEmptyTitle")}</h4><p>${t("hub.feedEmptyBody")}</p></div></div>`}
        </div>
    ...
    render(state) {
      ...
      <div class="ob" style="min-height:auto;background:transparent;padding:0">
        <div class="ob-card fade-up" style="max-width:680px;margin:0 auto">
          ...
          <div class="ob-body">
            ${role === "teacher" ? `
              <p class="eyebrow">${t("hub.obTeacherEyebrow")}</p>
              <h2 class="ob-title">${t("hub.obTeacherTitle")}</h2>
              <p class="muted" style="margin-bottom:18px">${t("hub.obTeacherBody")}</p>
              <div class="field" style="margin-bottom:14px"><label class="label">${t("hub.obHeadlineLabel")}</label><input class="input" id="ob-headline" placeholder="${t("hub.obHeadlinePlaceholder")}" value="${esc((DB.coachProfile && DB.coachProfile.headline) || "")}"/></div>
              <div class="field" style="margin-bottom:14px"><label class="label">${t("hub.obFormatsLabel")}</label><input class="input" id="ob-formats" placeholder="${t("hub.obFormatsPlaceholder")}" value="${esc((DB.coachProfile && DB.coachProfile.formats) || "")}"/></div>
