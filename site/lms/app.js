...
(function () {
  ...
  const ROUTES = {
    ...
    hub:          { screen:'hubHome',      title:'Hub',               crumbs:['Hub'] },
    explore:      { screen:'explore',      title:'Explorar',          crumbs:['Hub','Explorar'] },
    library:      { screen:'library',      title:'Biblioteca',        crumbs:['Hub','Biblioteca'] },
    'my-experience': { screen:'myExperience', title:'Mi experiencia',  crumbs:['Hub','Mi experiencia'] },
    'my-programs':   { screen:'myPrograms',   title:'Mis programas',   crumbs:['Hub','Mis programas'], role:'teacher' },
  };
  ...
  const NAV = {
    student: [
      { group:'Academia', items:[
        { r:'hub', ic:'grid', l:'Hub' },
        { r:'my-experience', ic:'sliders', l:'Mi experiencia' },
        { r:'explore', ic:'search', l:'Explorar' },
        { r:'library', ic:'file', l:'Biblioteca' },
      ]},
    ...
    teacher: [
      { group:'Academia', items:[
        { r:'hub', ic:'grid', l:'Hub' },
        { r:'my-programs', ic:'pencil', l:'Mis programas' },
        { r:'teacher-saul', ic:'user', l:'Mi perfil público' },
        { r:'library', ic:'file', l:'Biblioteca' },
      ]},
      { group:'Enseñar', items:[
  ...
  const TABBAR = {
    student: [ {r:'hub',ic:'grid',l:'Hub'},{r:'dashboard',ic:'home',l:'Aula'},{r:'course',ic:'book',l:'Curso'},{r:'progress',ic:'levels',l:'Niveles'},{r:'profile',ic:'user',l:'Perfil'} ],
    teacher: [ {r:'hub',ic:'grid',l:'Hub'},{r:'teacher',ic:'chart',l:'Panel'},{r:'gradebook',ic:'doc',l:'Notas'},{r:'participants',ic:'users',l:'Alumnos'},{r:'profile',ic:'user',l:'Perfil'} ],
  };
      ...
      <div class="login">
        ...
        <div class="login-form">
          <div class="lf-card">
            ...
            <p class="faint" style="text-align:center;margin-top:18px;font-size:12px">¿Nuevo en OTR? <a href="#onboarding">Crea tu perfil de estudiante o profesor</a></p>
          </div>
        </div>
      </div>`;
    drawLoginWave($('#lb-wave'));
    $('#login-form').addEventListener('submit', (e) => { e.preventDefault(); state.role='student'; location.hash = '#hub'; });
  ...
  function route() {
    ...
    if (hash === 'onboarding') {
      ...
      SCREENS.onboarding.renderInto($('#login-root'));
      window.scrollTo(0, 0);
      return;
    }
    if (hash.startsWith('teacher-') && hash !== 'teacher') {
      const tid = hash.slice(8);
      const t = (DB.teachers || []).find(x => x.id === tid);
      if (t) ROUTES[hash] = { screen: 'teacher_' + tid, title: t.name, crumbs: ['Hub', 'Explorar', t.name] };
    }
    if (hash === 'login' || hash === '') {
      $('#app').hidden = true;
      $('#login-root').hidden = false;
  ...
  function hashKey(screen) {
    if (screen.startsWith('teacher_')) return 'explore';
    ...
    const m = { dashboard:'dashboard', course:'course', courseIndex:'course', lesson:'course', assignment:'course',
      ...
      forum:'forum', forumThread:'forum', messages:'messages', hubHome:'hub', explore:'explore', library:'library',
      myExperience:'my-experience', myPrograms:'my-programs' };
