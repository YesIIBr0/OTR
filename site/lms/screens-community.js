/* OTR LMS · community screens: foro, hilo, mensajería */
(function () {
  ...
  S.forum = {
    render() {
      ...
      <button class="btn btn-primary" data-toast="Editor de discusión (demo)">${IC.plus} Nueva discusión</button></div>
  ...
  S.forumThread = {
    ...
    mount(root){
      ...
      send.addEventListener('click',()=>{
        ...
        window.toast && window.toast('Respuesta publicada ✓','ok'); box.value=''; box.classList.remove('err');
      });
    }
  };
  ...
  S.messages = {
    ...
    mount(root){
      ...
      const push=()=>{ const v=input.value.trim(); if(!v)return;
        ...
        div.innerHTML=`<div class="bubble">${v}<span class="b-time">ahora</span></div>`;
