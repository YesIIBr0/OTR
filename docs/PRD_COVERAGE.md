...
- **Diferidos con razón concreta (cramear = regresión, NO "bueno"):** FE-02 (trocear Aula.tsx — refactor mayor del dispatcher de toda la app, alto riesgo cross-feature), FE-04 (reemplazar `execCommand` — deprecado pero funcional en todos los navegadores hoy; reescribir el editor central de autoría necesita su propio esfuerzo + pruebas cross-browser), ENT-03 (búsqueda acento-insensible — requiere columna normalizada + **backfill** que el `prisma db push` del VPS no ejecuta), BE-04 (suite de tests — transversal), PRD-03 (reorder por flechas — el drag&drop ya es fluido), PRD-05 (consolidar Panel↔Builder). Cada uno merece su propio esfuerzo enfocado.
